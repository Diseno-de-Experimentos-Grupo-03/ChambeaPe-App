      ...
      body: FutureBuilder(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              if (users.isEmpty) {
                return const Center(
                    child: Text('Aún no has iniciado ningún chat'));
              }
  ...
  Future<void> loadMessageDetails() async {
    try {
      ...
      chatroomId = generateChatRoomId(
          currentUser!.id.toString(), users[otherUserIndex].id.toString());
      List<Message.ChatMessage> _lastMessage =
          await MessageService().getMessages(chatroomId, latest: true);
